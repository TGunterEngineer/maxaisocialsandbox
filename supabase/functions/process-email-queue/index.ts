...
function asEmailQueuePayload(value: Json): EmailQueuePayload | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null
  return value as EmailQueuePayload
}
...
async function moveToDlq(
  ...
  const payload = asEmailQueuePayload(msg.message)
...
Deno.serve(async (req) => {
  ...
  // Service-role only: compare the bearer token bytewise to the static
  // SUPABASE_SERVICE_ROLE_KEY. This avoids relying on unverified JWT claims
  // and matches the pattern used by other service-role-only edge functions.
  ...
  for (const queue of ['auth_emails', 'transactional_emails']) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      const payload = asEmailQueuePayload(msg.message)
      if (!payload) {
        await moveToDlq(supabase, queue, msg, 'Invalid email payload')
        continue
      }
